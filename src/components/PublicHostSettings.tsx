...
const organizationsByCity: Record<string, { name: string; address: string }[]> = {
  ...
  新北市: [{ name: "新北愛心協會", address: "新北市板橋區中山路 50 號" }],
  桃園市: [{ name: "桃園公益基金會", address: "桃園市中壢區中正路 100 號" }],
  台中市: [{ name: "台中希望協會", address: "台中市西屯區台灣大道 300 號" }],
  台南市: [{ name: "台南愛心基金會", address: "台南市中西區民生路 150 號" }],
  高雄市: [{ name: "高雄公益協會", address: "高雄市前鎮區成功路 200 號" }],
};
      ...
      <div className="flex-1 p-5 space-y-4">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={20}
              className={`text-muted-foreground transition-transform ${isCityOpen ? "rotate-180" : ""}`}
            />
          </button>
          {isCityOpen && (
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={20}
              className={`text-muted-foreground transition-transform ${isOrgOpen ? "rotate-180" : ""}`}
            />
          </button>
          {isOrgOpen && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
